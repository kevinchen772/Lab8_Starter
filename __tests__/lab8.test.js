describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartEle = await page.$("#cart-count");
    const innerText = await cartEle.getProperty('innerText');
    let value = await innerText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartEle = await page.$("#cart-count");
    const innerText = await cartEle.getProperty('innerText');
    let valuecart = await innerText.jsonValue();
    expect(valuecart).toBe('20');
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cartItem = await page.evaluate(() => {return localStorage.getItem('cart');}); 
